...
var dev = cli.Command{
	...
	Verbs: map[string]cli.Verb{
		"build":    {Run: stage.BuildVerb, Args: cli.DIR, Desc: "make the binary, and the manual when the command has verbs", Usage: stage.Usage},
		"wasm":     {Run: stage.WasmVerb, Args: cli.DIR, Flags: stage.EnvFlag, Desc: "build the same command as a Worker instead, for the environment you name", Usage: stage.Usage},
		"check":    {Run: stage.CheckVerb, Args: cli.DIR, Flags: stage.CheckFlags, Desc: "everything that says the command is sound; this is what CI runs", Usage: stage.Usage},
		"run":      {Run: stage.RunVerb, Args: cli.DIR + " [-- ARGS]", Desc: "run the built binary with the repo's secrets loaded", Usage: stage.Usage},
		"workerd":  {Run: stage.WorkerdVerb, Args: cli.DIR + " [-- ARGS]", Flags: stage.EnvFlag, Desc: "serve the Worker locally, the way Cloudflare will run it", Usage: stage.Usage},
		"deploy":   {Run: app.DeployVerb, Args: cli.DIR + " [-- FLAGS]", Flags: app.DeployFlags, Desc: "put the command in the cloud its directory names", Usage: app.Usage},
		"url":      {Run: app.URLVerb, Args: cli.DIR, Flags: app.URLFlags, Desc: "print the address to talk to, deployed or local", Usage: app.Usage},
		"logs":     {Run: app.LogsVerb, Args: cli.DIR, Flags: app.LogsFlags, Desc: "follow the deployed app's logs as they happen", Usage: app.Usage},
		"smoke":    {Run: app.SmokeVerb, Args: cli.DIR, Flags: app.SmokeFlags, Desc: "start the Worker locally and make one request, to know a build is not broken", Usage: app.Usage},
		"health":   {Run: app.HealthVerb, Args: cli.DIR, Flags: app.HealthFlags, Desc: "what the deployed app answers, and the headers it answers with", Usage: app.Usage},
		...
		"list":     {Run: app.ListVerb, Args: cli.DIR, Flags: app.EnvFlag, Desc: "what is deployed on this directory's cloud, with this one marked", Usage: app.Usage},
		"delete":   {Run: app.DeleteVerb, Args: cli.DIR, Flags: app.DeleteFlags, Desc: "remove a deployed app, and the storage created with it; asks first", Usage: app.Usage},
		"secrets":  {Subs: secrets.Subs, Usage: secrets.Usage},
		"session":  {Subs: session.Subs, Usage: session.Usage},
		"release":  {Run: release.Run, Args: cli.DIR + " [VERSION]", Flags: release.Flags, Desc: "build for every platform, sign it, and publish it to GitHub", Usage: release.Usage},
